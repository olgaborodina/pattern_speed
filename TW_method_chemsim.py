...
##########  for SBR
# snapshot.vx, snapshot.vy = np.zeros_like(snapshot.vx), np.zeros_like(snapshot.vy)
# snapshot.vx, snapshot.vy = f.add_solid_body_rotation(snapshot.x, snapshot.y, snapshot.vx, snapshot.vy, -omega)


##########  for smooth density
# density = snapshot.nCO
# radius_grid = np.linspace(0, 130, 201)
# density_grid = []
# radius_i_1 = 0

# for radius_i in np.concatenate((radius_grid[1:], [170])):
#     mask = (np.sqrt(snapshot.x ** 2 + snapshot.y ** 2) > radius_i_1) & (np.sqrt(snapshot.x ** 2 + snapshot.y ** 2) < radius_i)
#     density_grid.append(density[mask].mean())
#     radius_i_1 = radius_i

# plt.plot()
# inter_density = interpolate.interp1d(radius_grid, np.array(density_grid), fill_value='extrapolate')

# fig, ax = plt.subplots(figsize=(8, 6))
# ax.scatter(radius_grid / 10, np.array(density_grid))
# ax.set_yscale('log')
# ax.set_xlabel(r'$r$ [kpc]')
# ax.set_xlabel(r'density [$cm^{-2}$]')
# plt.savefig(FIG_DIR / 'density_mean.png', dpi=300)

# print('Calculated azimuthal mean')

# density_tot = inter_density(np.sqrt(snapshot.x ** 2 + snapshot.y ** 2))
...
w = (density_tot * snapshot.volumes) /(dx*dy)*(arepoLength) # weights for binning
name = 'All gas density'
...
sc = ax.scatter(bar.X_lon, bar.Y_lon, c=bar.Flux, 
                cmap='seismic', norm=norm, marker='s', s=2)
ax.grid(ls='dashed')
...
plt.colorbar(sc, format='%g', label=r'$\rm cm^{-2}$')
plt.savefig(FIG_DIR / '3D_galaxy_density_all.png', dpi=300)
...
for index, y in enumerate(bar.y_slits):
    if index % 5 == 0:
        ax1.plot([bar.X_lon.min(), bar.X_lon.max()], [y, y], c=colors[index], lw=1.2)
...
ax1.text(0.17, 0.89, r"$i=%.1f ^{\circ}$" % (np.rad2deg(i)) + " \n " \
                     + r"$\psi_{bar}=%.1f ^{\circ}$" % (np.rad2deg(psi_bar)), 
        ha="center", va="center", size=20, fontsize=16,
...
ax1.set_xlim(bar.X_lon.min(), bar.X_lon.max())
ax1.set_aspect("1")
